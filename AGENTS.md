...
5. Wait for `@edcalderon` approval, and keep patch coverage (new/changed lines in the PR) at or above 69% with the GitHub security scans passing before merging the PR. `main` is enforced by an active GitHub ruleset (id `18627241`) — direct pushes to `main` are rejected, so this PR is the only way in.
