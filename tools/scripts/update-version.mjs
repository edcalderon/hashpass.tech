  ...
  for (const target of VERSIONS_JSON_TARGETS) {
    ...
    if (historyMatch) {
      ...
      const versionsData = {
        ...
        _source: "This file is automatically generated from apps/web-app/config/version.ts",
        _instructions: "To update versions, edit apps/web-app/config/version.ts and run: npm run version:bump",
...
(async () => {
  if (allUpdated) {
    ...
    console.log('   - apps/web-app/config/versions.json');
    console.log('   - apps/web-app/config/git-info.json');
    console.log('   - CHANGELOG.md');
    console.log('   - apps/web-app/public/sw.js');
