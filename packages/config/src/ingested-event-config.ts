...
export function resolveNextOccurrence(
  ...
  if (Number.isNaN(next.getTime()))
    throw new Error(`Invalid ingested event date: ${event.startsAt}`);
...
export function getHashPokerEventConfig(now = new Date()): EventConfig | null {
  ...
  return {
    ...
    branding: {
      ...
      logo: event.organizerLogo || event.coverImage || "",
    },
    api: {
      basePath: "/api/events/hash-poker",
      endpoints: { agenda: "agenda", status: "status", checkin: "checkin" },
    },
    routes: {
      home: "/events/hash-poker/home",
    ...
    quickAccessItems: [
      {
        ...
        icon: "event",
        color: "#8B1538",
        route: "/events/hash-poker/agenda",
      },
      {
        id: "networking",
        ...
        icon: "people-alt",
        color: "#2563EB",
        route: "/events/hash-poker/networking",
      },
      {
        id: "pass",
        title: "Member Smart Pass",
        subtitle: "Identity & QR check-in",
        icon: "wallet",
        color: "#059669",
        route: "/(shared)/dashboard/wallet",
      },
      {
        id: "info",
        title: "Hash Poker Room",
        subtitle: "Host & house information",
        icon: "info",
        color: "#D97706",
        route: "/events/hash-poker/event-info",
      },
    ],
    sourceId: event.sourceId,
    organizerName: event.organizerName,
    communityEventType: event.eventType as EventConfig["communityEventType"],
    recurrenceLabel: "Weekly Poker Room",
    cta: event.cta || undefined,
    networkingEnabled: event.networkingEnabled,
