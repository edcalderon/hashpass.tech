#!/bin/bash
# Script to create IAM role for GitHub Actions infra deployments
...
echo ""
if [ -z "$REPO" ]; then
    echo "ERROR: Set REPO or GITHUB_REPOSITORY before running this script."
    exit 1
fi
...
aws iam create-role \
  ...
  --description "Role for GitHub Actions to deploy HashPass infra" \
  --region $REGION

echo "✅ Role created"

echo "📝 Attaching AWS permissions..."
aws iam attach-role-policy \
  --role-name $ROLE_NAME \
  --policy-arn arn:aws:iam::aws:policy/AdministratorAccess \
  --region $REGION

DELETE_PROTECTION_POLICY=$(cat <<EOF
{
  "Version": "2012-10-17",
  "Statement": [
    {
...
aws iam put-role-policy \
  --role-name $ROLE_NAME \
  --policy-name BslHashpassDeleteProtection \
  --policy-document "$DELETE_PROTECTION_POLICY" \
  --region $REGION
...
echo "      Value: $ROLE_ARN"
echo ""
echo "   3. Push to main/develop to trigger .github/workflows/infra-deploy.yml"
echo ""
