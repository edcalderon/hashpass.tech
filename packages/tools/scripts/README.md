...
- `pnpm run android:release:alpha` submits the production build path to the Play Console alpha closed-testing track and is blocked until a successful internal release exists for the same tag
...
- The self-hosted workflow lives in `.github/workflows/mobile-android-release.yml`, expects the `hashpass-mobile-release` AWS runner label, and now accepts Play track and release-status inputs for production or closed-testing releases. Closed alpha cannot run until internal has already succeeded for the same tag.
