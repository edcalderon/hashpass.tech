...
if [[ -z "${STACK}" ]]; then
  echo "Usage: $0 <aws|gcp|mobile-release-target|mobile-release-legacy-source-account|hashpass-dns|hashpass-api-target|hashpass-web> [plan|apply|destroy|validate|fmt] [var-file]"
  exit 1
fi
