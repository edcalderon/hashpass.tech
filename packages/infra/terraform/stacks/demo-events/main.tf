...
locals {
  ...
  cbw2026_site_bucket_name   = "hashpass-cbw2026-develop-site-${data.aws_caller_identity.current.account_id}-${var.aws_region}"
  cbw2026_site_origin_domain = "${local.cbw2026_site_bucket_name}.s3-website.${var.aws_region}.amazonaws.com"
  ...
  # real failed plan).
  cbw2026_artifact_bucket_name = "hashpass-cbw2026-pipelines-${data.aws_caller_identity.current.account_id}-${var.aws_region}"

  demo_event_origin_domain_names = {
    cbw2026 = local.cbw2026_site_origin_domain
  }
}
