...
This stack's state tracks the **old source AWS account (`<source-account-id>`)**,
...
`<target-account-id>`, instance confirmed live and building releases as of
...
(`<target-account-id>`, per `AWS_TARGET_ACCOUNT_ID` in the repo-root `.env`), not the
...
needs valid AWS credentials for account `<source-account-id>` specifically — none
