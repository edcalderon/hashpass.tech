...
const postJson = async <T>(path: string, body: Record<string, unknown>): Promise<{ success: boolean; data?: T; error?: string }> => {
  try {
...
export const authenticateWithEthereum = async (): Promise<{ userId: string; email: string; walletAddress: string; tokenHash?: string | null; magicLink?: string | null; redirectUrl?: string | null }> => {
  ...
  const challengeResult = await postJson<{ nonce: string; message: string }>('/api/auth/wallet/challenge', {
    walletAddress,
    walletType: 'ethereum',
  });
  ...
  const authResult = await postJson<{ userId: string; email: string; walletAddress: string; tokenHash?: string | null; magicLink?: string | null; redirectUrl?: string | null }>('/api/auth/wallet/ethereum', {
...
- ${origin}`;
  ...
  // Step 5: Verify signature and authenticate
  const authResult = await postJson<{ userId: string; email: string; walletAddress: string; tokenHash?: string | null; magicLink?: string | null; redirectUrl?: string | null }>('/api/auth/wallet/solana', {
