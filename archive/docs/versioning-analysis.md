...
- `apps/mobile-app/config/version.ts` → `buildNumber`, `releaseDate`, `releaseType`, `notes`, `VERSION_HISTORY`
- `apps/mobile-app/config/versions.json` → `currentVersion`, `versions[]`
- `apps/mobile-app/config/git-info.json` → `gitCommit`, `gitCommitFull`, `gitBranch`, `gitRepoUrl`
