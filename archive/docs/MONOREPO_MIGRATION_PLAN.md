...
hashpass.tech/
├── apps/
│   ├── mobile-app/           # Expo Router mobile/PWA app
...
#### `apps/mobile-app`
- **Purpose**: Main Expo Router mobile/PWA application
- **Contains**:
...
#### `apps/web-app`
- **Purpose**: Standalone Next.js web app for `hashpass.club`
- **Contains**:
  - `app/` - Next.js app router pages and layout
  - Club-specific UI composition
  - Shared `@hashpass/ui` primitives
- **Dependencies**: Shared packages needed by the club web runtime
...
1. **Move the Expo app to `apps/mobile-app`**
   ```bash
   mv app apps/mobile-app/app
   mv components apps/mobile-app/components  # App-specific only
   mv hooks apps/mobile-app/hooks            # App-specific only
   ```
...
Migration executed per `.agents/task`. Root app remains runnable; `apps/mobile-app` is the migrated app using `@hashpass/*` packages; `apps/directus` holds SSO Docker setup; Terraform moved to `packages/infra/terraform`.
