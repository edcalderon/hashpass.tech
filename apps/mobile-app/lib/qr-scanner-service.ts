...
import { BarCodeScanner, type BarCodeScannerResult, type BarcodeType } from './barcode-scanner-compat';
...
class QRScannerService {
  ...
  isSupported(): boolean {
    if (Platform.OS === 'web') {
      // On web, check both the native camera wrapper and ZXing fallback
      return (
  ...
  /**
   * Enable web fallback (for when the native camera scanner fails on web)
   */
