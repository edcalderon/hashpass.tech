...
function readRequest(body: unknown, headers: Record<string, string> = {}) {
  return new Request(
    `https://api.hashpass.tech/api/v1/support/tickets/${TICKET_ID}/read`,
    {
      method: "POST",
      headers: {
...
function handoffRequest(headers: Record<string, string> = {}) {
  return new Request(
    `https://api.hashpass.tech/api/v1/support/tickets/${TICKET_ID}/handoff`,
    {
      method: "POST",
...
describe("POST /api/v1/support/tickets/:ticketId/read", () => {
  ...
  it("returns 401 with no session", async () => {
    ...
    const {
      POST,
    } = require("../../../../app/api/v1/support/tickets/[ticketId]/read+api");
  ...
  it("marks the ticket read at the given cursor", async () => {
    ...
    expect(mockRpc).toHaveBeenCalledWith(
      "mark_ticket_read",
      expect.objectContaining({ p_cursor: "cursor-1" }),
    );
  });
...
describe("POST /api/v1/support/tickets/:ticketId/handoff", () => {
  ...
  it("returns 401 with no session", async () => {
    ...
    const {
      POST,
    } = require("../../../../app/api/v1/support/tickets/[ticketId]/handoff+api");
  ...
  it("requests handoff and notifies the support team when configured", async () => {
    ...
    expect(body).toEqual(
      expect.objectContaining({ id: TICKET_ID, status: "open" }),
    );
