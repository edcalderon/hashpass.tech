...
function getRequest(query = "") {
  return new Request(
    `https://api.hashpass.tech/api/v1/support/tickets/${TICKET_ID}/messages${query}`,
    {
...
function postRequest(body: unknown, headers: Record<string, string> = {}) {
  return new Request(
    `https://api.hashpass.tech/api/v1/support/tickets/${TICKET_ID}/messages`,
    {
      method: "POST",
      headers: {
...
describe("GET/POST /api/v1/support/tickets/:ticketId/messages", () => {
  ...
  it("returns 401 with no session", async () => {
    ...
    const {
      GET,
    } = require("../../../../app/api/v1/support/tickets/[ticketId]/messages+api");
  ...
  it("sends a message and replays an identical response for a repeated Idempotency-Key", async () => {
    ...
    const first = await POST(
      postRequest({ body: "reply" }, { "idempotency-key": "k1" }),
    );
