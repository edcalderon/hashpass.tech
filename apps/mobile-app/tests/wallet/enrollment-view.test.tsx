...
const row = { scope: { ownerId: 'owner-a', walletId: 'wallet-a', environment: 'development' }, state: 'enrolled', wallet: null, setupEnabled: false };
...
it('ignores stale responses and rejects data belonging to another identity', async () => {
  let finish!: (value: unknown) => void;
  mockLoad.mockReturnValueOnce(new Promise(resolve => { finish = resolve; })).mockResolvedValueOnce(row);
  await act(async () => { view = create(<WalletEnrollmentView />); });
  mockUserId = 'owner-b';
