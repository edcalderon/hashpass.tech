...
jest.mock("../../hooks/useAuth", () => ({
  useAuth: () => ({
    isLoggedIn: true,
    dbUserId: "viewer-1",
    user: { email: "ed@example.com", user_metadata: { full_name: "Edward Calderon" } },
  }),
}));
jest.mock("../../i18n/i18n", () => ({
  useTranslation: () => ({
    t: (
      ...
      const fallback = typeof fallbackOrParams === "string" ? fallbackOrParams : _key;
...
jest.mock("../../lib/event-chat", () => ({
  ...
  getEventChatUpgradeUrl: (eventId: string, tier: string) => `https://tickets.test/${eventId}?tier=${tier}`,
  heartbeatEventChatPresence: (...args: unknown[]) => mockHeartbeat(...args),
  loadEventChat: (...args: unknown[]) => mockLoadEventChat(...args),
  loadEventChatMembers: (...args: unknown[]) => mockLoadEventChatMembers(...args),
  sendEventChatMessage: (...args: unknown[]) => mockSendEventChatMessage(...args),
}));
...
async function pressLabel(renderer: TestRenderer.ReactTestRenderer, label: string) {
  const target = renderer.root.findAll((node: any) => node.props.accessibilityLabel === label)[0];
  await act(async () => {
...
async function pressText(renderer: TestRenderer.ReactTestRenderer, label: string) {
  const target = renderer.root.findAllByType("Text" as any).find((node: any) =>
    String(node.props.children ?? "") === label,
  );
...
describe("EventRoomChat", () => {
  ...
  it("shows the entry rules, supports anonymous identity, and renders zero live attendees", async () => {
    ...
    const renderer = await renderRoom({ eventId: "colombia2026", eventTitle: "Blockchain Summit Latam Colombia 2026" });
    ...
    expect(textContent(renderer)).toContain("General pass: you can read this room");
    expect(mockLoadEventChat).toHaveBeenCalledWith("colombia2026", "room", undefined);
  ...
  it("sends replies and emojis, while opening attendee profiles for direct-message upgrades", async () => {
    ...
    const renderer = await renderRoom({ eventId: "colombia2026", eventTitle: "Colombia 2026" });
    await enterRoom(renderer);
    ...
    const input = renderer.root.find((node: any) => node.props.accessibilityLabel === "Message");
    ...
    expect(mockSendEventChatMessage).toHaveBeenCalledWith(
      expect.objectContaining({ message: "Thanks", replyToMessageId: "message-1" }),
    );
  ...
  it("loads VIP members and sends a direct message to the selected attendee", async () => {
    mockLoadEventChat.mockResolvedValue({
      ...baseData,
      passType: "vip",
      permissions: { canReadRoom: true, canSendRoom: true, canSendDirect: true },
    });
    mockLoadEventChatMembers.mockResolvedValue([
      { userId: "attendee-2", name: "Business Attendee", passType: "business", avatarUrl: null },
    ]);
    const renderer = await renderRoom({ eventId: "colombia2026", eventTitle: "Colombia 2026" });
    await enterRoom(renderer);
    ...
    await pressLabel(renderer, "Send message");
    expect(mockSendEventChatMessage).toHaveBeenCalledWith(
      expect.objectContaining({ eventId: "colombia2026", message: "Private hello", recipientId: "attendee-2" }),
    );
    await unmountRoom(renderer);
  });

  it("replaces the protected route with Explore when the attendee check is denied", async () => {
    const denied = Object.assign(new Error("No active event pass"), { status: 403 });
