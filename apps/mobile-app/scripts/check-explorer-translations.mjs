...
const getValue = (messages, key) => {
  ...
  const walk = (value, index) => {
    ...
    if (Object.prototype.hasOwnProperty.call(value, remaining))
...
const catalogs = Object.fromEntries(
  requiredLocales.map((locale) => [locale, readLocale(locale)]),
);
const errors = [];
...
const sourceKeys = new Set(
  [
    ...explorerSource.matchAll(
      /\btranslate\(\s*["'](explore\.rework\.[^"']+)["']/g,
    ),
  ].map((match) => match[1]),
);
...
console.log(
  `Verified dashboard translations for ${requiredLocales.join(", ")}.`,
);
