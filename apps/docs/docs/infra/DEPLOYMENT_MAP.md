...
| `bsl.hashpass.tech` | **Cutover in progress (2026-07-29)** — still served by source-account CloudFront `E2FCDJB1JCS7TW` with SST's `placeholder.sst.dev` origin; target-account hybrid infra applied but origin not yet repointed | Static (Expo web export) | us-east-2 | Currently the source-account `bsl-hashpass-prod` pipeline (`sst deploy`). Target-account `bsl-hashpass-prod` is switched to the hybrid script and building — see the BSL section below for the remaining cutover steps |
...
**`bsl.hashpass.tech` (prod): cutover started 2026-07-29, NOT finished.** What's done: the target-account S3 bucket (`bsl-hashpass-bsl-prod-site-<target-account-id>-us-east-2`) exists, and `bsl-target`'s `bsl-hashpass-prod` pipeline is switched from `build-bsl-infra.sh` (SST) to `build-bsl-static-site.sh` (hybrid). What's **not** done: `E2FCDJB1JCS7TW`'s origin is still SST's `placeholder.sst.dev`, so the **source-account pipeline is still the live serving path**. The origin repoint is deliberately gated on the new bucket actually having content — repointing to an empty bucket would blank the live site. Full ordered checklist: "Remaining steps to finish BSL prod" in `.agents/active/task-aws-account-migration.md`.
