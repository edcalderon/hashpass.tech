// Chapter seek-points for the app-tutorial demo videos. Computed from the
// real recording durations (minus each clip's trimStartSeconds) plus the
// 3s intro bumper — see apps/video-studio/src/content/clips.ts for the
// source values and apps/video-studio/src/lib/clip-layout.ts for the same
// math Remotion uses when it renders the actual composition. Both language
// cuts (silent and narrated) share identical timing since narration audio
// was sized to fit inside each clip's existing duration, not the other way
// around, so one chapter list works for all four App Tutorial videos.
export type Chapter = {
  slug: string;
  titleKey: string;
  startSeconds: number;
...
export const chaptersEs: Chapter[] = [
  {slug: 'landing', titleKey: 'chapterLanding', startSeconds: 3},
