...
const assetsDir = path.join(root, 'assets/openproof');
const publicDir = path.join(root, 'apps/web-app/public/openproof');
const svg = path.join(assetsDir, 'openproof-architecture.svg');
const diagram = path.join(assetsDir, 'openproof-architecture.png');
const thumbnail = path.join(assetsDir, 'openproof-thumbnail.png');
const video = path.join(assetsDir, 'openproof-walkthrough.mp4');

await Promise.all([mkdir(assetsDir, {recursive: true}), mkdir(publicDir, {recursive: true})]);
...
console.log(`OpenProof exports written to ${assetsDir} and ${publicDir}`);
