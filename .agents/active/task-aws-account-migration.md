...
| BSL (`bsl.hashpass.tech` / `bsl-dev.hashpass.tech`) | CodeBuild `bsl-hashpass-prod-build`/`bsl-hashpass-dev-build` + CodePipeline `bsl-hashpass-dev`/`bsl-hashpass-prod`, both wired to `edcalderon/hashpass.tech` (the wrong repo — see incident below), still running but stale | **Migrated (2026-07-28).** New Terraform stack `packages/infra/terraform/stacks/bsl-target` provisions a dedicated EC2 build worker + `bsl-hashpass-prod`/`bsl-hashpass-dev` CodePipelines, correctly wired to `hashpass-tech/hashpass.tech`. See "BSL pipeline incident and migration" below for the full story. |
...
**DNS**: confirmed the deploy is safe to validate without touching live traffic — `packages/tools/scripts/check-infra-dns.sh` resolves whichever Route53 zone exists in the *current* AWS account, so a target-account deploy only touches the target zone's own (non-authoritative) copy of `hashpass.tech`. The real, publicly-resolving source zone is untouched until the `bsl.hashpass.tech`/`bsl-dev.hashpass.tech` A/AAAA records are deliberately updated in the *source* zone to alias the new target-account CloudFront distribution — the exact same source-zone-alias-to-target-resource pattern `hashpass.tech`/`dev.hashpass.tech` already use. That DNS cutover is the last step, done only after both pipelines are validated end to end.

**Superseded, not yet cleaned up**: the source-account `bsl-hashpass-prod`/`bsl-hashpass-dev` CodePipelines and CodeBuild projects, and the target-account `bsl-hashpass-prod-build`/`bsl-hashpass-dev-build` CodeBuild projects (no longer used now that the EC2-worker pipelines exist). Left alone for now — matches this task's own "don't destroy until the replacement is proven" principle.
