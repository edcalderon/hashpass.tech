-- accept_meeting_request wrote slot_status = 'confirmed' into
-- user_agenda_status, but that column's own CHECK constraint
-- (user_agenda_status_slot_status_check) only ever allowed
-- 'available' | 'interested' | 'blocked' | 'tentative' -- 'confirmed' has
-- never been a legal value. This was unreachable in practice until V046
-- fixed the identity-space mismatch on the FK targets these inserts also
-- hit, so it's only now surfacing as the next real blocker -- confirmed by
-- an end-to-end test against live dev data. status = 'confirmed' (a
-- different column, with its own CHECK that does allow 'confirmed')
-- already carries the actual confirmation signal, and the agenda-conflict
-- detection used elsewhere in this function keys off meeting_id being set,
-- not slot_status -- so the fix is simply to stop writing an invalid
-- slot_status value on confirm, not to relax the constraint. A NULL
-- slot_status satisfies the CHECK.
