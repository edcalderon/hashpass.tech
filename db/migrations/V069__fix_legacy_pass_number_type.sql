-- V069: support legacy integer pass numbers in the backend-only pass RPC
--
-- Development tenants created from the original core schema use a serial
-- integer for passes.pass_number. Production tenants use text. The RPC must
-- leave integer columns to their sequence default and only write the display
-- number when the column is text-backed.
