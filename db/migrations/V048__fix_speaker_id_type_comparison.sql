-- Both get_speaker_available_slots and accept_meeting_request compared
-- meetings.speaker_id directly against v_speaker.id with no cast.
-- get_speaker_by_id_or_slug always returns id as text (it has to, since it
-- uniformly handles both text-slug and uuid-typed bsl_speakers.id across
-- tenants -- confirmed dev uses text slugs like 'edward-calderon', prod
-- uses real uuids), but meetings.speaker_id is uuid on prod. The bare `=`
-- has no uuid/text operator, so this was a hard parse-time error on any
-- tenant where bsl_speakers.id is a real uuid -- confirmed live via an
-- end-to-end test against prod, not merely theoretical. Cast the uuid side
-- to text so the comparison works regardless of which type a given
-- tenant's bsl_speakers.id actually is.
