...
-- confirmed via explicit, deliberate comments in both my-schedule.tsx and
-- agenda/status+api.ts, which is how real users actually mark availability
-- and confirm agenda items today. V046 pointed this FK at auth.users
-- instead, reasoning from accept_meeting_request's own (auth-id) writes
-- alone without checking the other real callers of this same column --
-- wrong, and a live regression on production for exactly the write paths
-- V046 didn't check. Revert the FK, and instead resolve both parties' auth
-- ids into their registry ids inside accept_meeting_request before every
-- read or write against this table, matching how my-schedule.tsx and
-- agenda/status+api.ts already resolve identity for it. When no registry
-- row exists yet for a party, the agenda-status read/write for that party
-- is skipped rather than failing the whole accept -- meetings/meeting_slots
-- (auth-id keyed, unaffected by this migration) remain the actual booking
-- record and conflict guarantee.
