-- ============================================================================
-- V033: Let event administrators manage BSL speaker account assignments
-- ============================================================================
-- A speaker assignment is a tenant-level identity link because the legacy BSL
-- directory stores a speaker's account on bsl_speakers.user_id. The event id
-- scopes who may perform the administrative action and is retained in the
-- audit log. It does not turn a speaker profile into a cross-event staff role.
-- ============================================================================
...
BEGIN
  ...
  IF p_action = 'grant' THEN
    ...
    SELECT id, lower(email)
     ...
     LIMIT 1;
    IF NOT FOUND THEN
      RAISE EXCEPTION 'No account exists for this email' USING ERRCODE = '22023';
    END IF;
    ...
    -- Preserve the assignment in the claim lifecycle so an auth-user deletion
    -- cleanly releases the speaker identity rather than leaving a stale link.
