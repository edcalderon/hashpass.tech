...
## Recent Fixes
- v1.8.278 (pending): BSL On Tour hero banner, event-list thumbnails, and carousel logo slides rendered blank on native (fine on web) — root cause: several BSL-only image assets were `require()`'d as raw `.svg` directly into React Native's `<Image>`, which has no SVG decoder on native (Android/iOS), unlike browsers. Rasterized the six affected SVGs to `.webp`/`.png` via `sharp` and repointed `event-branding.ts`/`EventBannerCarousel.tsx`; kept the `.svg` string lookup keys unchanged since those are identifiers, not asset paths. See [svg-native-image-rendering-gotcha.md](apps/docs/docs/reference/mobile-app/svg-native-image-rendering-gotcha.md).
